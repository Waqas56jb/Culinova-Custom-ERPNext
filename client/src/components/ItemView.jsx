...
import { specGroups } from '@shared/specs.js'
    ...
    <Modal open={open} onClose={onClose} size="lg" title={it?.item_name || 'Item'} subtitle={it ? `${it.item_code}${it.product_family ? ` · ${it.product_family}` : ''}` : ''}
      ...
      {it && (
        <div className="space-y-4">
          ...
          {(() => {
            const groups = specGroups(it.specifications)
            if (!groups.length) return null
            return (
              <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-3">
                <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-slate-400">Specifications</p>
                <div className="space-y-3">
                  {groups.map((g) => (
                    <div key={g.group}>
                      {g.group !== 'other' && (
                        <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-brand-600">{g.label}</p>
                      )}
                      <dl className="grid grid-cols-1 gap-x-4 gap-y-1 sm:grid-cols-2">
                        {g.rows.map((row, idx) => (
                          <div key={`${row.name}-${idx}`} className="flex justify-between gap-2 border-b border-slate-100/80 py-1 text-sm">
                            <dt className="text-muted">{row.name}</dt>
                            <dd className="font-medium text-ink text-right">{row.value}{row.unit ? ` ${row.unit}` : ''}</dd>
                          </div>
                        ))}
                      </dl>
                    </div>
          ...
          {alts.length > 0 && (
            <div className="rounded-xl border border-slate-200 p-3">
              ...
              {alts.map((a) => (
                <div key={a.id || a.item_id} className="flex items-start justify-between gap-3 border-b border-slate-50 py-2 text-sm last:border-0">
                  <div className="min-w-0">
                    <p className="font-medium text-ink">{a.item_name} <span className="text-xs text-muted">· {a.brand}</span></p>
                    {(() => {
                      const chips = (a.reasons?.length ? a.reasons : [a.reason || 'Same product family']).filter(Boolean)
                      return chips.length > 0 ? (
                        <p className="mt-0.5 flex flex-wrap gap-1">
                          {chips.map((c) => (
                            <span key={c} className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600">{c}</span>
                          ))}
                        </p>
                      ) : null
                    })()}
                    <p className="mt-0.5 text-[11px] text-slate-400">
                      Available {a.available ?? a.available_qty ?? '—'}
                      {(a.incoming ?? a.incoming_qty) > 0 ? ` · Incoming ${a.incoming ?? a.incoming_qty}` : ''}
                    </p>
                  </div>
                  <span className="shrink-0 font-semibold text-brand-600">{sar(a.selling_price ?? a.standard_rate ?? 0)}</span>
                </div>
              ))}
            </div>
          )}
