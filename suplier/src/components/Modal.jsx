...
export function Field({ label, hint, ...props }) { return <label className="block">{label && <span className="mb-1.5 block text-xs font-semibold text-slate-600">{label}</span>}<input {...props} className={inputCls} />{hint && <span className="mt-1 block text-[11px] text-muted">{hint}</span>}</label> }
