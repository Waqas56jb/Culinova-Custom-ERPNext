...
export async function runEosSync({ trigger = 'auto' } = {}) {
  ...
  // a sync that changed nothing is not worth a log line every 30 minutes
...
export function startEosAutoSync() {
  ...
  timer = setInterval(async () => {
    elapsed += 1
    const mins = Number(await setting('eos_auto_sync_minutes', '30')) || 30
    if (elapsed >= mins) { elapsed = 0; await tick() }
  }, 60_000)
  timer.unref?.()
  console.log('[eos-sync] automatic EOS → Item Master sync armed (interval from system_settings)')
}
