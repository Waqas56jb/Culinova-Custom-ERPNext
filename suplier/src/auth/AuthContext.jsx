...
export function AuthProvider({ children }) {
  ...
  async function login(email, password) { const { token, user: u } = await api('/auth/login', { method: 'POST', auth: false, body: { email, password } }); if (u.role !== PORTAL_ROLE) throw new Error('This portal is for suppliers only.'); persist(token, u) }
