...
// The Document Manager is an Administration feature (Sidebar → Administration → Documents). Gate the
// WHOLE module on the admin panel: without this, any authenticated user — including a portal Customer
// with zero panels — could list, download and DELETE every internal company document.
r.use(authRequired)

// ── DOCUMENTS with version history ──
// Documents can be attached to any entity (entity_type + entity_id) or stand alone.
r.get('/', authorize('admin', 'read'), asyncWrap(async (req, res) => {
...
r.get('/:id', authorize('admin', 'read'), asyncWrap(async (req, res) => {
...
r.post('/', authorize('admin', 'create'), asyncWrap(async (req, res) => {
  const b = req.body
...
r.post('/:id/version', authorize('admin', 'update'), asyncWrap(async (req, res) => {
...
r.delete('/:id', authorize('admin', 'delete'), asyncWrap(async (req, res) => {
