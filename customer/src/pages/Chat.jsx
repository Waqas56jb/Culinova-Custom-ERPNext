...
export default function Chat() {
  ...
  // light polling so sales replies appear without a manual refresh
    ...
    <>
      ...
      <div className="card flex h-[calc(100vh-220px)] flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 p-4">
          <span className="grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br from-navy-700 to-brand-600 text-sm font-bold text-white">{initials(user?.name)}</span>
          <div className="min-w-0">
            <p className="font-bold text-ink">{user?.name}</p>
            <p className="flex items-center gap-1 text-xs text-muted"><Mail size={12} /> {user?.email}</p>
          </div>
          <span className="ml-auto rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600">CULINOVA Sales</span>
        </div>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50/50 p-4">
          ...
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.sender === 'customer' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-3.5 py-2 text-sm ${m.sender === 'customer' ? 'bg-brand-500 text-white' : 'bg-white text-ink shadow-soft'}`}>
        ...
        <div className="flex items-center gap-2 border-t border-slate-100 p-3">
          <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && send()}
            placeholder="Type your message…" className="flex-1 rounded-xl border border-slate-200 bg-slate-50/70 px-3.5 py-2.5 text-sm outline-none focus:border-brand-400 focus:bg-white" />
