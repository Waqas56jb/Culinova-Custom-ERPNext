...
import { Check, X as XIcon, MessageCircle, Loader2, FileText, Trash2 } from 'lucide-react'
...
export default function Quotations() {
  ...
  const Btn = ({ onClick, tone = 'slate', icon: Icon, children, id }) => (
    <button onClick={onClick} disabled={busy === id}
  ...
  return (
    <>
      <PageHeader title="Quotations" subtitle="Review quotes from CULINOVA — view, accept, reject, request a better price, or delete" />
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-1.5">
                      ...
                      {q.status === 'Open' && (
                        <>
                          ...
                          <Btn onClick={() => reject(q)} tone="rose" icon={XIcon} id={q.id}>Reject</Btn>
                        </>
                      )}
                      {q.status !== 'Ordered' && <Btn onClick={() => remove(q)} tone="rose" icon={Trash2} id={q.id}>Delete</Btn>}
                    </div>
                  </td>
                </tr>
              ))}
