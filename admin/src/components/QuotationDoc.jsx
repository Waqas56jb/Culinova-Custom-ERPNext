...
export default function QuotationDoc({ open, onClose, q }) {
  ...
  const items = (q.quotation_items || []).map((it, i) => ({ idx: i + 1, name: it.item_name, qty: it.qty, rate: it.rate, amount: it.amount || it.qty * it.rate }))
  const net = Number(q.net_amount) || items.reduce((s, it) => s + it.amount, 0)
  const disc = Number(q.discount_amount) || 0
  const vat = Number(q.vat_amount) || 0
  const total = Number(q.total_amount) || 0

  return (
    <div className="fixed inset-0 z-[95] flex items-start justify-center overflow-y-auto bg-navy-900/60 p-3 backdrop-blur-sm sm:p-6">
      <div className="w-full max-w-3xl animate-fade-up">
        <div className="mb-3 flex items-center justify-between text-white">
          <span className="flex items-center gap-2 text-sm font-semibold"><FileText size={16} /> {q.number}.pdf</span>
        ...
        <div className="overflow-hidden rounded-xl bg-white shadow-2xl">
          <div className="flex items-start justify-between bg-gradient-to-br from-navy-900 to-navy-700 p-7 text-white">
            ...
            <div className="text-right">
              ...
              <p className="text-xs text-slate-300">{q.number}</p>
            </div>
          </div>
          ...
          <div className="p-7">
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                {q.project_location && <p className="text-xs text-slate-500">{q.project_location}</p>}
              ...
              <div className="text-right text-xs text-slate-500">
                ...
                <p><span className="font-semibold text-ink">Discount:</span> {q.discount_pct}%</p>
            ...
            <div className="mt-7 border-t border-slate-100 pt-4 text-[11px] leading-relaxed text-slate-500">
              ...
              <p>• Prices are in SAR and inclusive of 15% VAT. • Payment: {q.payment_terms || '50% advance, 50% on delivery'}. {q.delivery_date ? `• Required delivery by ${q.delivery_date}.` : ''} • Warranty: 12 months on supplied equipment.</p>
            </div>
          </div>
        </div>
      </div>
