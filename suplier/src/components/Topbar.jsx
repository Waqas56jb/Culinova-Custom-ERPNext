...
export default function Topbar({ onMenu }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[68px] items-center gap-3 border-b border-slate-200/70 bg-white/80 px-4 backdrop-blur-md lg:px-7">
      ...
      <p className="hidden text-sm text-muted sm:block">Supplier portal · <span className="font-semibold text-ink">{user?.name}</span></p>
      <div className="ml-auto flex items-center gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2.5 rounded-xl border border-slate-200 bg-white py-1.5 pl-1.5 pr-2.5">
          ...
          <div className="hidden text-left leading-tight sm:block"><p className="text-[13px] font-semibold text-ink">{user?.name}</p><p className="text-[11px] text-muted">Supplier</p></div>
        </div>
