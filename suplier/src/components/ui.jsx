...
const accentMap = { brand: 'from-brand-500 to-brand-600', violet: 'from-violet-500 to-indigo-600', gold: 'from-gold-500 to-gold-600', emerald: 'from-emerald-500 to-teal-600' }

export function PageHeader({ title, subtitle, children }) {
  return (
    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between animate-fade-up">
      <div><h1 className="font-display text-2xl font-extrabold text-ink tracking-tight">{title}</h1>{subtitle && <p className="mt-1 text-sm text-muted">{subtitle}</p>}</div>
...
export function ChartCard({ title, subtitle, action, children, className = '' }) {
  return (
    <div className={`card card-pad animate-fade-up ${className}`}>
      <div className="mb-4 flex items-center justify-between"><div><h3 className="text-[15px] font-bold text-ink">{title}</h3>{subtitle && <p className="text-xs text-muted">{subtitle}</p>}</div>{action}</div>
      {children}
    </div>
  )
}
...
export function statusTone(s) {
  const map = {
    Open: 'blue', Quoted: 'amber', Awarded: 'green', Ordered: 'green',
    Pending: 'amber', Received: 'green', Billed: 'violet',
    Preparing: 'gray', Shipped: 'blue', Delivered: 'green',
  }
  return map[s] || 'gray'
}
